...
from models.pretrained_vmamba_model import get_vmamba
def has_CONCH():
    HAS_CONCH = False
    CONCH_CKPT_PATH = ''
...
def has_vit():
    ...
    try:
        # check if UNI_CKPT_PATH is set
        if 'vit_CKPT_PATH' not in os.environ:
            raise ValueError('vit_CKPT_PATH not set')
        HAS_vit = True
        vit_CKPT_PATH = os.environ['vit_CKPT_PATH']
    except Exception as e:
        print(e)
    return HAS_vit, vit_CKPT_PATH
from models.vmamba_models import build_model
def get_encoder(model_name, target_img_size=224):
    ...
    if model_name == 'resnet50_trunc':
        ...
        HAS_vit, vit_CKPT_PATH = has_vit()
        model = timm.create_model('vit_small_patch16_224',num_classes=0)
        model.load_state_dict(torch.load(vit_CKPT_PATH, map_location="cpu"), strict=False)
    else:
